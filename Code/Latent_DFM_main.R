    ...
    for (k in 1:4){ # d
      for (l in 1:2){ # T
        ...
        if (j == 1){
          ...
          rho1 <- 0.2; rho2 <- 0.3; rho3 <- 0.4
          rho0 <- 1-ARMAacf(ar=c(rho1,rho2,rho3))[2:4] %*% c(rho1,rho2,rho3)
          
          model_list <- list()
          model_list$Psi <- array(cbind(diag(rho1,r),diag(rho2,r),diag(rho3,r)),
                                  dim=c(r,r,p))
          model_list$Cov_eta <- diag(as.numeric(rho0),r)
        ...
        sim_link <- list()
        sim_cov_Z <- list()
        sim_cv_p_lowdim <- list()
        sim_cv_p_Gaussian <- list()
        sim_trad_p <- list()
        sim_cv_p <- list()
        N_sim <- 100
        cat("#----------------------------------# \n")
        ...
        for (iter in 1:N_sim){
          ...
          while (again){
            ...
            if (i == 1){
              if (sum(rowSums(DGP$X_t[,-c(TT+1:(TT+H))]) == 0) == 0 
                  & sum(rowSums(DGP$X_t[,-c(TT+1:(TT+H))]) == TT) == 0){
                again <- FALSE
              }
            }else if (i == 2){
              possible_num <- c(rep(5,d/3),rep(3,d/3),rep(3,d/3))
              if (sum(mapply(x=1:d,function(x)
                sum(seq(1,5) %in% unique(DGP$X_t[x,-c(TT+1:(TT+H))])))!=possible_num)==0){
                again <- FALSE
              }
            }else { # i == 3 | i == 4
              if (sum(rowSums(DGP$X_t[,-c(TT+1:(TT+H))]) == 0) == 0){
          ...
          #---------------------------------------------------#
          # estimating p (low-dimensional Gaussian):
          Y_t <- DGP$Y_t[,-c(TT+1:(TT+H))]
          cv_p_lowdim <- Latent_DFM_p_Selection_BCV_Gaussian_lowdim(TT,r,Y_t,fold=4)
          sim_cv_p_lowdim[[iter]] <- cv_p_lowdim
          
          #---------------------------------------------------#
          # estimating p (Gaussian):
          Z_t <- DGP$Z_t[,-c(TT+1:(TT+H))]
          cv_p_Gaussian <- Latent_DFM_p_Selection_BCV_Gaussian(d,TT,r,Z_t,identy_opt=1,fold=4)
          sim_cv_p_Gaussian[[iter]] <- cv_p_Gaussian
          ...
          trad_p <- Latent_DFM_p_Selection_trad(d,TT,r,X_t,
                                                sim_link[[iter]]$Link,identy_opt=1)
          sim_trad_p[[iter]] <- trad_p
          cv_p <- Latent_DFM_p_Selection_BCV(d,TT,r,X_t,
                                             sim_link[[iter]]$Link,identy_opt=1,fold=4)
          sim_cv_p[[iter]] <- cv_p
        }
        cat("Setting ",i,j,k,l," ends \n")
        cat("#----------------------------------# \n")
        
        save_fn <- paste0("p_est","_dist",i,"_type",j,"_d",k,"_T",l,".rda")
        path_p <- dir_p
        save(sim_cv_p_lowdim,sim_cv_p_Gaussian,sim_trad_p,sim_cv_p,
             file = file.path(path_p,save_fn))
      }
    }
  }
}
  ...
  for (j in 1:2){ # lag
    for (k in 1:4){ # d
      for (l in 1:2){ # T
        ...
        # load file:
        cat("Setting ",i,j,k,l," begins \n")
        load_fn <- paste0("p_est","_dist",i,"_type",j,"_d",k,"_T",l,".rda")
        path_p <- dir_p
        load(paste0(path_p,"/",load_fn))
        ...
        N_sim <- 100
        IC_1 <- unlist(mapply(x=1:N_sim,function(x)sim_trad_p[[x]]$p_IC1))
        IC_2 <- unlist(mapply(x=1:N_sim,function(x)sim_trad_p[[x]]$p_IC2))
        IC_3 <- unlist(mapply(x=1:N_sim,function(x)sim_trad_p[[x]]$p_IC3))
        IC_4 <- unlist(mapply(x=1:N_sim,function(x)sim_trad_p[[x]]$p_IC4))
        Lowdim <- unlist(mapply(x=1:N_sim,function(x)sim_cv_p_lowdim[[x]]$p_PC))
        Gaussian <- unlist(mapply(x=1:N_sim,function(x)sim_cv_p_Gaussian[[x]]$p_PC))
        BCV <- unlist(mapply(x=1:N_sim,function(x)sim_cv_p[[x]]$p_PC))
        
        df_temp <- data.frame(
          
          dist = rep(dist,N_sim*7),
          lag = rep(p,N_sim*7),
          dim = rep(d,N_sim*7),
          length = rep(TT,N_sim*7),
          p_hat = c(IC_1,IC_2,IC_3,IC_4,Lowdim,Gaussian,BCV),
          methods = c(rep("IC1",N_sim),
                      rep("IC2",N_sim),
                      rep("IC3",N_sim),
                      rep("IC4",N_sim),
...
df_total$length <- factor(df_total$length,levels=unique(df_total$length),
                          labels=c("T=100","T=200"))
df_total$lag <- factor(df_total$lag,levels=unique(df_total$lag),
                       labels=c("p=1","p=3"))
df_total$methods <- factor(df_total$methods,levels=unique(df_total$methods))
df_total$dist <- factor(df_total$dist,levels=unique(df_total$dist))
...
names(length.labs) <- c(TeX("$T=100$"),TeX("$T=200$"))
lag.labs <- c("p=1","p=3")
names(lag.labs) <- c(TeX("$p=1$"),TeX("$p=3$"))


# Plotting:
pl_bern <- ggplot(df_total[df_total$dist=="Bern",],aes(x=p_hat,fill=methods)) +
  geom_histogram(alpha=0.5,bins=11,position='dodge') +
  scale_x_continuous(breaks=c(0:5)) +
  xlab(TeX("$\\hat{p}$")) +
  ylab("Frequency") + 
  scale_fill_manual(name="",values=rev(colorRampPalette(brewer.pal(9,"Set1"))(7)),
                    labels=levels(df_total$methods)) +
  facet_nested(lag~length+dim,
               labeller=labeller(length=length.labs,dim=dim.labs,
                                 lag=lag.labs)) + 
  ggtitle("Bernoulli distributions") +
  theme(legend.position = "bottom",
...
pl_multi <- ggplot(df_total[df_total$dist=="multinom",],aes(x=p_hat,fill=methods)) +
  geom_histogram(alpha=0.5,bins=11,position='dodge') +
  scale_x_continuous(breaks=c(0:5)) +
  xlab(TeX("$\\hat{p}$")) +
  ylab("Frequency") + 
...
pl_pois <- ggplot(df_total[df_total$dist=="Pois",],aes(x=p_hat,fill=methods)) +
  geom_histogram(alpha=0.5,bins=11,position='dodge') +
  scale_x_continuous(breaks=c(0:5)) +
  xlab(TeX("$\\hat{p}$")) +
  ylab("Frequency") + 
...
pl_negbin <- ggplot(df_total[df_total$dist=="negbin",],aes(x=p_hat,fill=methods)) +
  geom_histogram(alpha=0.5,bins=11,position='dodge') +
  scale_x_continuous(breaks=c(0:5)) +
  xlab(TeX("$\\hat{p}$")) +
  ylab("Frequency") + 
...
pdf(width=9,height=12,file = "lag_estim.pdf")
par(mar=c(0,0,0,0))
ggarrange(pl_bern,pl_multi, pl_pois,pl_negbin,
          ncol=1, nrow=4, common.legend = TRUE, legend="bottom")
dev.off()
